...
(a) Como o código acima executa o debounce do botão? Justifique sua resposta.
	...
	Nao acontece debounce, por conta do While(test), pois ele um buffer para esperar o tempo do btn estabilizar.

(b) Em quais bordas (subida/descida) é feito o debounce? Justifique sua resposta.

	Não dar para saber, pois a função poll_btn não há diferenciação

(c) Qual é a utilidade da variável 'test' dentro da função 'poll_btn()'? Justifique sua resposta.

	test é uma variavel booleana que determina o fim do laço. 
	
(d) Por que a função 'lseek()' é sempre chamada antes da função 'read()' dentro da função 'poll_btn()'? Justifique sua resposta.
	
	É nessesario voltar o ponteiro para o inicio, que é a pasta do gpio4.
...
void loop(int sig)
{
	...
	do
	{
		txrx = 0xA5; // envio para msp430
		wiringPiSPIDataRW(0, &txrx, 1);
	} while(txrx != 0x5A); // envio para rasp
	usleep(100);
	txrx = 1;
	wiringPiSPIDataRW(0, &txrx, 1);
...
(a) Qual é a sequência de bytes pré-estabelecida para o Raspberry Pi receber uma leitura A/D? Como o MSP430 responde?
	...
	O Raspberry Pi envia 0xA5 para o MSP430 (10100101). O MSP430 responde com 0x5A (01011010)

(b) Qual é a taxa de amostragem das leituras A/D? Justifique.
	
	Ele precisa de 500us como ele pega dois 2 byts por vez temos 2000 amostras.
...
(d) Quantas médias das amostras serão apresentadas no terminal? Justifique.
	...
	Por conta do usleep de 100 iremos pegar apenas apenas 50 amostras.Eles vai mostrar. 
